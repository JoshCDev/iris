"""One-off audit of the IRIS rice ONNX vs PhyToSignal training set.

Does not modify models. Writes experiments/outputs/vision_audit.json.
"""
...
def main() -> None:
    report: dict = {"dataset_root": str(DATA), "dataset_exists": DATA.is_dir()}
    if not DATA.is_dir():
        OUT.parent.mkdir(parents=True, exist_ok=True)
        OUT.write_text(json.dumps(report, indent=2), encoding="utf-8")
        print("missing dataset", DATA)
        return
    ...
    def eval_preprocess(fn, max_per_class: int = 80) -> dict:
        ...
        for true_i, cls in enumerate(CLASSES):
            files = list_images("test", cls)[:max_per_class]
            for path in files:
    ...
    n_pair = 0
    for true_i, cls in enumerate(CLASSES):
        for path in list_images("test", cls)[:40]:
