...
The image classifier was trained on public rice-leaf photographs: Sethy’s four-disease image dataset [10], [11] and a subset of Paddy Doctor images for overlapping disease classes and healthy leaves [13]. Indonesian field leaves have not been evaluated. The training images and split manifest are not committed, so the reported split cannot be independently reconstructed from this repository alone.

A logistic regression was fitted to daily precipitation returned by the Open-Meteo Historical Weather API for Salatiga [14] (1 Jan. 2018-26 Aug. 2026; _n_ = 3,154). It estimates whether day _i_ plus the next two days sum to ≥15 mm. In-sample accuracy is 0.5891 versus a wet base rate of 0.5022; there is no held-out test. These are project-run statistics, not claims made by Open-Meteo. The score is never used to withhold irrigation. The scheduler still uses the BMKG 72 h total [12]. If the score disagrees with BMKG, or lies between 0.35 and 0.65, the interface flags the forecast for human review.
